...
>
> **Update 2026-09-09 (Tier 2, PRs #12–#14) — RBAC is now feature-complete on both sides.** `KICK_MEMBERS` is a real permission (`POST /community/kick/:communityId`, narrower than `MANAGE_MEMBERS`, Moderator-accessible). Member **role assignment**: `POST /role/assign` + `/role/unassign` (`MANAGE_ROLES`; owner's roles locked; member keeps ≥1 role), with a per-member checkbox editor in the member modal. **Roles CRUD UI**: `RolesModalComponent` from the About page — list/create/edit/delete, defaults read-only. `deleteRole` cascades to member `roleIds`. Shared `PERMISSIONS` constant (server + client, client has display labels). Member list shows coloured role pills; member modal shows Kick or Remove per the viewer's permission. **Update (PR #19): `VIEW_CONTENT` / `SEND_MESSAGES` are now enforced.** `VIEW_CONTENT` gates seeing channels (`getAccessibleChannels`/`searchAccessibleChannels` return empty) and joining/reading/sending in a channel chat; `SEND_MESSAGES` gates posting (both first- and subsequent-message paths), so the default **Guest** role can read a channel but not post. DMs unchanged. `ChatUseCase` now takes `IRBACService`; its "Unauthorized to …" throws are now `UnauthorizedError` (401, were 500). **Update (PR #21): the PR #19 follow-ups are done.** Reactions and poll votes in a community channel now require `SEND_MESSAGES` (`assertCanParticipateWithMessage`); voiceroom join (REST token/presence + socket `room:watch`/`room:join`) now requires `VIEW_CONTENT` — `userHasChannelAccess`/`assertVoiceroomChannelAccess` take an optional `rbacService`, `VoiceroomUseCase` gets it as a dep. `chat.usecase` client-fault errors are now `CustomError` subclasses (404/400/401, were 500). DMs still unaffected by any of this.
