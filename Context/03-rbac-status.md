...
| `MANAGE_MEMBERS` | `approveJoinRequest`, `rejectJoinRequest`, `addMember`, `removeMember` | Correct in the usecase; `approveJoinRequest`/`rejectJoinRequest` are unreachable due to the controller argument-swap bug (`04-known-bugs.md` #1); `removeMember` is unreachable because the UI calls the wrong service method |
...
- **`MANAGE_TAG` is required by `addTag`/`removeTag` but granted by no predefined role.** `community.usecase.ts` checks `hasPermission(userId, communityId, "MANAGE_TAG")` for both, but `defaultRolesData` in `predifinedRoles.ts` never lists `MANAGE_TAG` on any of the five roles — not even `Owner`. As shipped, **no one can add or remove a tag on an existing community**, community creator included, unless an admin first uses the (currently nonexistent) custom-role UI to hand-craft a role with that permission string. This is very likely an oversight (forgot to add `MANAGE_TAG` to `Owner`/`Admin`) rather than intentional.
...
- **No protection for the `Owner` role.** `updateRole`/`deleteRole` block edits to *any* `isDefault` role (`role.isDefault` check), which is good, but nothing stops `removeMember` (once its argument-order bug is fixed) from removing the sole `Owner`, or `leaveCommunity` from letting the owner leave, leaving the community with no one holding `MANAGE_COMMUNITY`/`MANAGE_ROLES`.
